    ...
    public SqlCondition caseCommonQuery (CaseQuery query) {
        return new SqlCondition().build((condition, args) -> {
            ...
            if ( query.getManagerIds() != null && !query.getManagerIds().isEmpty() ) {
                ...
                if (!isWithoutManager) {
                    condition.append(" and manager IN " + HelperFunc.makeInArg(managerIds, false));
                } else if (managerIds.isEmpty()) {
                    condition.append(" and manager IS NULL");
                } else {
                    condition.append(" and (manager IN " + HelperFunc.makeInArg(managerIds, false) + " OR manager IS NULL)");
                }
            }
