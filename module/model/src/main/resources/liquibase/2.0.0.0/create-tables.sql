...
ALTER TABLE admin_state COMMENT = 'Административное состояние (locked/unlocked)';
...
ALTER TABLE attachment COMMENT = 'Вложения, которые используются на разных уровнях';
...
ALTER TABLE auth_type COMMENT = 'Тип авторизации клиентов портала (LDAP, персональный пароль,';
...
ALTER TABLE case_attachment COMMENT = 'Список вложений CASE';
...
ALTER TABLE case_comment COMMENT = 'Комментарии к CASE';
...
ALTER TABLE case_comment_reply COMMENT = 'Список обратных ссылок для сборки ответов на определенный ко';
...
ALTER TABLE case_doc_type COMMENT = 'Типы документов для CASE';
...
ALTER TABLE case_document COMMENT = 'Документы для Case';
...
ALTER TABLE case_event COMMENT = 'События для CASE';
...
ALTER TABLE case_event_type COMMENT = 'Типы событий для Case';
...
ALTER TABLE case_object COMMENT = 'Subj';
...
ALTER TABLE case_refs COMMENT = 'Кросс-ссылки CASE друг на друга';
...
ALTER TABLE case_stage COMMENT = 'Этапы для CASE. Каждый этап - это группа из CASE(Этап';
...
ALTER TABLE case_stage_type COMMENT = 'Словарь возможных этапов для CASE';
...
ALTER TABLE case_state COMMENT = 'Пополняемый словарь возможных статусов для CASE. Словарь сод';
...
ALTER TABLE case_state_matrix COMMENT = 'матрица определяющая возможные наборы статусов для каждого т';
...
ALTER TABLE case_task COMMENT = 'Линейный список задач по каждому CASE';
...
ALTER TABLE case_term COMMENT = 'Различные сроки по Case (дедлайны)';
...
ALTER TABLE case_term_type COMMENT = 'Типы сроков по Case';
...
ALTER TABLE case_time_log COMMENT = 'Журнал учета времени по задачам для Case';
...
ALTER TABLE case_type COMMENT = 'Типы CASE';
...
ALTER TABLE case_vroom COMMENT = 'Виртуальная комната для общения, позволяет создать определен';
...
ALTER TABLE company COMMENT = 'Компании';
...
ALTER TABLE company_dep COMMENT = 'Подразделения компании';
...
ALTER TABLE company_group_home COMMENT = 'Список компаний, сотрудники которых считаются "своими" и име';
...
ALTER TABLE department_type COMMENT = 'Тип подразделения';
...
ALTER TABLE dev_unit COMMENT = 'Элементы разработки :- компоненты (бывшие проекты .CP';
...
ALTER TABLE dev_unit_att COMMENT = 'Вложения для компоненты/продукта';
...
ALTER TABLE dev_unit_branch COMMENT = 'Ветка разработки';
...
ALTER TABLE dev_unit_children COMMENT = 'Дочерние элементы продукта/компоненты';
...
ALTER TABLE dev_unit_member COMMENT = 'Участники продукта/компоненты';
...
ALTER TABLE dev_unit_type COMMENT = 'Тип единицы разработки (Текущий словарь из 2 элементов: комп';
...
ALTER TABLE dev_unit_version COMMENT = 'Версия';
...
ALTER TABLE importance_level COMMENT = 'Уровень важности для объектов системы (для CASE в частности)';
...
ALTER TABLE person COMMENT = 'Контакты, включая внутренних сотрудников компании';
...
ALTER TABLE person_absence COMMENT = 'Журнал отсутствий';
...
CREATE TABLE sys_config (id BIGINT NOT NULL, ownCompanyId BIGINT NOT NULL COMMENT 'Идентификатор компании-владельца базы портал.
            Сотрудники этой компании имеют право получать доступ к основным ресурсам сервера', CONSTRAINT PK_SYS_CONFIG PRIMARY KEY (id)) COMMENT='Системная конфигурация';
ALTER TABLE sys_config COMMENT = 'Системная конфигурация';
...
ALTER TABLE time_unit COMMENT = 'Словарь едениц измерений для времени';
...
ALTER TABLE user_login COMMENT = 'логины для авторизации';
...
ALTER TABLE user_role COMMENT = 'Роль авторизации, каждой роли соответствует определенный наб';
...
ALTER TABLE worker_entry COMMENT = 'Кадровая запись, единица штатного расписания.';
...
ALTER TABLE worker_position COMMENT = 'Должности для сотрудников';
...
ALTER TABLE case_comment ADD CONSTRAINT FK_CASECOMMENT_STATE FOREIGN KEY (CSTATE_ID) REFERENCES case_state (ID) ON UPDATE NO ACTION ON DELETE NO ACTION;
ALTER TABLE case_comment ADD CONSTRAINT FK_CASECOMM_VROOM FOREIGN KEY (VROOM) REFERENCES case_vroom (ID) ON UPDATE NO ACTION ON DELETE NO ACTION;
...
ALTER TABLE case_comment ADD CONSTRAINT FK_CASE_COMMENT FOREIGN KEY (CASE_ID) REFERENCES case_object (ID) ON UPDATE NO ACTION ON DELETE NO ACTION;
...
ALTER TABLE case_comment ADD CONSTRAINT FK_CSCOMM_AUTHOR FOREIGN KEY (AUTHOR_ID) REFERENCES person (id) ON UPDATE NO ACTION ON DELETE NO ACTION;
ALTER TABLE case_comment ADD CONSTRAINT FK_CSCOMM_REPLY FOREIGN KEY (REPLY_TO) REFERENCES case_comment (ID) ON UPDATE NO ACTION ON DELETE CASCADE;
ALTER TABLE case_comment_reply ADD CONSTRAINT FK_CSC_REF FOREIGN KEY (CASE_ID) REFERENCES case_object (ID) ON UPDATE NO ACTION ON DELETE CASCADE;
ALTER TABLE case_comment_reply ADD CONSTRAINT FK_CSC_REF_REPLY FOREIGN KEY (REPLY_ID) REFERENCES case_comment (ID) ON UPDATE NO ACTION ON DELETE CASCADE;
ALTER TABLE case_comment_reply ADD CONSTRAINT FK_CSC_REF_SRC FOREIGN KEY (COMMENT_ID) REFERENCES case_comment (ID) ON UPDATE NO ACTION ON DELETE NO ACTION;
...
ALTER TABLE case_attachment ADD CONSTRAINT FK_PARENT_COMMENT FOREIGN KEY (CCOMMENT_ID) REFERENCES case_comment (ID) ON UPDATE NO ACTION ON DELETE CASCADE;
...
ALTER TABLE case_comment ADD CONSTRAINT case_comment_remote_link_id_fk FOREIGN KEY (remote_link_id) REFERENCES case_link (id) ON UPDATE NO ACTION ON DELETE NO ACTION;
...
ALTER TABLE case_comment ADD CONSTRAINT cmanager_id_to_person_fk FOREIGN KEY (cmanager_id) REFERENCES person (id) ON UPDATE CASCADE ON DELETE NO ACTION;
...
ALTER TABLE case_comment ADD CONSTRAINT fk_case_comment_importance FOREIGN KEY (cimp_level) REFERENCES importance_level (id) ON UPDATE NO ACTION ON DELETE NO ACTION;
