...
<#noparse><#if is_created == true>${_reservedIpActionCreated}</#if><#if is_updated == true>${_reservedIpActionUpdated}</#if><#if is_removed == true>${_reservedIpActionRemoved}</#if> [${initiator}]</#noparse>