...
<table style="width:70%">
    ...
    <#list reservedIps as reservedIp>
        <tr>
            <td class="ip" colspan="3">
                ${reservedIp.ipAddress}
                <#if reservedIp.macAddress??>&nbsp;[${reservedIp.macAddress}]</#if>
            </td>
        </tr>
        <tr>
            <td class="field">${reservedIp.ownerShortName}</td>
            <td class="field">
                ${reservedIp.reserveDate?string["dd.MM.yyyy"]} - ${reservedIp.releaseDate?string["dd.MM.yyyy"]}
            </td>
