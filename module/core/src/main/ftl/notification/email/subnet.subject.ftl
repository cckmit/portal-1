...
<#noparse><#if is_created == true>${_subnetActionCreated}</#if><#if is_updated == true>${_subnetActionUpdated}</#if><#if is_removed == true>${_subnetActionRemoved}</#if> ${_subnetAction} ${subnetIpAddress} [${initiator}]</#noparse>
