...
public class ContractorsSelector extends Composite implements AbstractContractorsSelector, HasEnabled, HasValue<Set<PersonProjectMemberView>> {
    ...
    public void setValue(Set<PersonProjectMemberView> value, boolean fireEvents) {
        ...
        model.clear();
        modelToView.clear();
        ContractorsSelectorItemModel.toModel(value).forEach(this::addNewItem);
    ...
    public Set<PersonProjectMemberView> getValue() {
        return ContractorsSelectorItemModel.fromModel(model);
    }
    ...
    @Override
    public void onModelChanged(ContractorsSelectorItemModel itemModel) {
        if (itemModel == null) {
            return;
        }
    ...
    @Override
    public void onRoleChanged(ContractorsSelectorItemModel target, En_PersonRoleType previous, En_PersonRoleType actual) {
        for (Map.Entry<ContractorsSelectorItemModel, AbstractContractorsSelectorItem> entry : modelToView.entrySet()) {
            ContractorsSelectorItemModel itemModel = entry.getKey();
            AbstractContractorsSelectorItem itemView = entry.getValue();
    ...
    private void addNewItem(ContractorsSelectorItemModel itemModel) {
        ...
        AbstractContractorsSelectorItem itemView = createItemView();
        itemView.setAvailableRoles(availableRoles);
    ...
    private void addNewEmptyItemIfNeeded() {
        if (!isEmptyItemExists()) {
            addNewItem(new ContractorsSelectorItemModel(null, true));
        }
    }

    private void removeItem(ContractorsSelectorItemModel itemModel) {
        AbstractContractorsSelectorItem itemView = modelToView.get(itemModel);
    ...
    private List<En_PersonRoleType> getAvailableRoles() {
        List<En_PersonRoleType> roles = listOf(En_PersonRoleType.getCardBatchRoles());
        model.forEach(itemModel -> roles.remove(itemModel.role));
        return roles;
    }

    private AbstractContractorsSelectorItem createItemView() {
        AbstractContractorsSelectorItem itemView = itemFactory.get();
    ...
    @Inject
    Provider<AbstractContractorsSelectorItem> itemFactory;

    private Set<ContractorsSelectorItemModel> model = new HashSet<>();
    private Map<ContractorsSelectorItemModel, AbstractContractorsSelectorItem> modelToView = new HashMap<>();
    private boolean isEnabled = true;

    interface ContractorsSelectorUiBinder extends UiBinder<HTMLPanel, ContractorsSelector> {}
    private static ContractorsSelectorUiBinder ourUiBinder = GWT.create(ContractorsSelectorUiBinder.class);
}
