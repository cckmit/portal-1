...
public class DeliveryFilterParamView extends Composite implements AbstractDeliveryFilterParamView {
    ...
    public void init() {
        ...
        fillDateRanges(dateDeliveryRange);
        fillDateRanges(dateModifiedRange);
        dateDeliveryRange.setHeader(lang.created());
    ...
    public void setCreatedRangeMandatory(boolean isMandatory) {
        dateDeliveryRange.setTypeMandatory(isMandatory);
    }

    @Override
    public void setCreatedRangeValid(boolean isTypeValid, boolean isRangeValid) {
        dateDeliveryRange.setValid(isTypeValid, isRangeValid);
    }
...
//        state.setValue(toSet(caseQuery.getStateIds(), id -> new CaseState(id)));
    ...
    @UiHandler({"dateDeliveryRange", "dateModifiedRange"})
    ...
    public void applyVisibility(En_DeliveryFilterType filterType) {
        ...
        searchFavoriteContainer.setVisible(filterType.equals(En_DeliveryFilterType.DELIVERY_OBJECTS));
        searchByComments.setVisible(filterType.equals(En_DeliveryFilterType.DELIVERY_OBJECTS));
        if (filterType.equals(En_DeliveryFilterType.DELIVERY_OBJECTS)) {
            ...
            dateDeliveryRange.setHeader(lang.period());
            dateModifiedRange.addStyleName(HIDE);
            sortByContainer.addClassName(HIDE);
        }
        creators.setVisible(!isCustomer && filterType.equals(En_DeliveryFilterType.DELIVERY_OBJECTS));
        initiators.setVisible((!isCustomer || !isSubcontractor) && filterType.equals(En_DeliveryFilterType.DELIVERY_OBJECTS));
        platforms.setVisible(!isCustomer && filterType.equals(En_DeliveryFilterType.DELIVERY_OBJECTS));
        managerCompanies.setVisible(filterType.equals(En_DeliveryFilterType.DELIVERY_OBJECTS));
        managers.setVisible((!isCustomer || isSubcontractor) && filterType.equals(En_DeliveryFilterType.DELIVERY_OBJECTS));
        searchPrivateContainer.setVisible(!isCustomer && filterType.equals(En_DeliveryFilterType.DELIVERY_OBJECTS));
        plan.setVisible(!isCustomer && filterType.equals(En_DeliveryFilterType.DELIVERY_OBJECTS));
        workTriggers.setVisible(!isCustomer && filterType.equals(En_DeliveryFilterType.DELIVERY_OBJECTS));
        overdueDeadlinesContainer.setVisible(!isCustomer && filterType.equals(En_DeliveryFilterType.DELIVERY_OBJECTS));
    }
    ...
    public boolean isCreatedRangeTypeValid() {
        return !dateDeliveryRange.isTypeMandatory()
               || (dateDeliveryRange.getValue() != null
                   && dateDeliveryRange.getValue().getIntervalType() != null);
    }

    public boolean isCreatedRangeValid() {
        return isDateRangeValid(dateDeliveryRange.getValue());
    }
    ...
    private void ensureDebugIds() {
        ...
        dateDeliveryRange.setEnsureDebugId(DebugIds.FILTER.DATE_CREATED_RANGE_CONTAINER);
    ...
    TypedSelectorRangePicker dateDeliveryRange;
    @Inject
    @UiField(provided = true)
