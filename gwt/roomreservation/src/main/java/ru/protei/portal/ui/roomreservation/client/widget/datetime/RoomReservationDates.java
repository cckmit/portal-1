...
import ru.protei.portal.ui.roomreservation.client.widget.datetime.item.RoomReservationDatesItem;
...
public class RoomReservationDates extends Composite implements HasValue<List<DateInterval>>, HasEnabled, HasVaryAbility {
    ...
    public List<DateInterval> getValue() {
        return stream(items)
            .map(RoomReservationDatesItem::getValue)
    ...
    private void fillView(List<DateInterval> intervals) {
        ...
        stream(intervals)
            .map(interval -> {
                RoomReservationDatesItem item = itemProvider.get();
    ...
    @Inject
    Provider<RoomReservationDatesItem> itemProvider;
    @UiField
    HTMLPanel root;
    @UiField
    HTMLPanel controls;
    ...
    private List<RoomReservationDatesItem> items = new ArrayList<>();
    private boolean isEnabled = true;
    private boolean isVaryAble = true;

    interface RoomReservationDatesBinder extends UiBinder<HTMLPanel, RoomReservationDates> {}
    private static RoomReservationDatesBinder ourUiBinder = GWT.create(RoomReservationDatesBinder.class);
}
